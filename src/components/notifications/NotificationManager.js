...
class NotificationManager {
    ...
    isEnabled() {
        ...
        return this.settings.enabled && this.permission === 'granted' && this.isSupported;
    }
    ...
    updateFaviconBadge(count) {
        try {
            ...
            logger.error('Failed to update favicon badge:', error);
        }
    }
}
...
export const useNotifications = () => {
    ...
    const mute = useCallback((duration = 60 * 60 * 1000) => {
        // Default 1 hour
...
export const NotificationSettings = ({ settings, onUpdate }) => {
    ...
    return (
        <div style={styles.container}>
            <h3 style={styles.title}>{t('🔔_notification_settings')}</h3>
            <label style={styles.label}>
                ...
                />
                <span>{t('notificationsi_etkinleştir')}</span>
            </label>
            <label style={styles.label}>
                <input
                    type="checkbox"
            ...
            <label style={styles.label}>
                ...
                />
                <span>{t('masaüstü_notificationsi')}</span>
            </label>
            <label style={styles.label}>
                <input
                    type="checkbox"
            ...
            <label style={styles.label}>
                ...
                />
                <span>{t('sadece_dm_ler')}</span>
            </label>

            <div style={styles.muteButtons}>
                <button
                    onClick={() =>
                        onUpdate({ ...settings, muteUntil: Date.now() + 15 * 60 * 1000 })
                    }
                ...
                <button
                    onClick={() =>
                        onUpdate({ ...settings, muteUntil: Date.now() + 60 * 60 * 1000 })
                    }
                    style={styles.muteButton}
                >
                    1 Saat Mute
                ...
                <button
                    onClick={() =>
                        onUpdate({ ...settings, muteUntil: Date.now() + 8 * 60 * 60 * 1000 })
                    }
                    style={styles.muteButton}
                >
                    8 Saat Mute

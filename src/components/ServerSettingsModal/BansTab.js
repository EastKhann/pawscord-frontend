        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '16px 20px', backgroundColor: '#0d0e10', borderRadius: '12px' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                    <FaBan style={{ fontSize: '24px', color: '#f23f42' }} />
                    <div>
                        <h3 style={{ margin: 0, color: '#fff', fontSize: '16px' }}>Yasaklı Kullanıcılar</h3>
                        <p style={{ margin: '2px 0 0', color: '#b5bac1', fontSize: '12px' }}>{bans.length} yasaklı kullanıcı</p>
                    </div>
                </div>
            ...
            {bansLoading ? (
                ...
                <div style={{ textAlign: 'center', padding: '60px 20px', backgroundColor: '#111214', borderRadius: '12px' }}>
                    <FaCheck style={{ fontSize: '48px', color: '#23a559', marginBottom: '16px' }} />
                    <h4 style={{ color: '#fff', margin: '0 0 8px' }}>Temiz!</h4>
                    <p style={{ color: '#b5bac1', fontSize: '14px' }}>Henüz yasaklanmış kullanıcı yok.</p>
                </div>
            ) : (
                <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                    {bans.map(ban => (
                        <div key={ban.id} style={{
                            ...
                            padding: '14px 18px', backgroundColor: '#111214', borderRadius: '8px',
                            border: '1px solid #0b0e1b', transition: 'background-color 0.15s'
                        }}>
                            <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                                <div style={{
                                    width: '36px', height: '36px', borderRadius: '50%',
                                    backgroundColor: '#f23f42', display: 'flex', alignItems: 'center',
                                ...
                                <div>
                                    ...
                                    <div style={{ color: '#4e5058', fontSize: '11px', marginTop: '2px' }}>
                                        ...
                                        {ban.expires_at && !ban.is_permanent && (
                                            <span style={{ marginLeft: '8px', color: '#f0b232' }}>⏰ Bitiş: {new Date(ban.expires_at).toLocaleDateString('tr-TR')}</span>
                                        )}
                                        {ban.is_permanent && (
                                            <span style={{ marginLeft: '8px', color: '#f23f42' }}>♾️ Kalıcı</span>
                                        )}
                                    </div>
                                </div>
                            </div>
                            <button onClick={() => handleUnban(ban.username)}
                                style={{ padding: '8px 14px', backgroundColor: 'transparent', border: '1px solid #23a559', borderRadius: '6px', color: '#23a559', cursor: 'pointer', fontWeight: '600', fontSize: '12px', display: 'flex', alignItems: 'center', gap: '6px', transition: 'all 0.15s' }}
                                onMouseEnter={(e) => { e.target.style.backgroundColor = '#23a559'; e.target.style.color = '#fff'; }}
                                onMouseLeave={(e) => { e.target.style.backgroundColor = 'transparent'; e.target.style.color = '#23a559'; }}
                            >

        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '16px 20px', backgroundColor: '#0d0e10', borderRadius: '12px' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                    <FaHistory style={{ fontSize: '24px', color: '#5865f2' }} />
                    <div>
                        <h3 style={{ margin: 0, color: '#fff', fontSize: '16px' }}>Aksiyon Geçmişi</h3>
                        <p style={{ margin: '2px 0 0', color: '#b5bac1', fontSize: '12px' }}>Tüm moderatör ve admin aksiyonları</p>
                    </div>
                </div>
                <div style={{ display: 'flex', gap: '8px' }}>
                    <select value={auditFilter} onChange={(e) => { setAuditFilter(e.target.value); loadAuditLogs(e.target.value); }}
                        style={{ padding: '8px 12px', backgroundColor: '#111214', border: '1px solid #182135', borderRadius: '6px', color: '#dbdee1', fontSize: '12px', outline: 'none', cursor: 'pointer' }}>
            ...
            {auditLoading ? (
                ...
                <div style={{ textAlign: 'center', padding: '60px 20px', backgroundColor: '#111214', borderRadius: '12px' }}>
                    ...
                    <p style={{ color: '#b5bac1', fontSize: '14px' }}>Henüz kayıtlı aksiyon bulunamadı.</p>
                </div>
            ) : (
                <div style={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
                    {auditLogs.map((log, idx) => {
                        const actionColors = {
                            'BAN': '#f23f42', 'UNBAN': '#23a559', 'KICK': '#f0b232',
                            'MESSAGE_DELETE': '#f47b67', 'ROLE_CHANGE': '#5865f2',
                            'CHANNEL_CREATE': '#23a559', 'CHANNEL_DELETE': '#f23f42',
                            'SERVER_UPDATE': '#5865f2', 'EMAIL_VERIFIED': '#23a559'
                        };
                        const actionColor = actionColors[log.action_type] || '#b5bac1';
                        return (
                            <div key={log.id || idx} style={{
                                display: 'flex', alignItems: 'center', gap: '12px',
                                padding: '12px 16px', backgroundColor: '#111214',
                                ...
                                <div style={{ flex: 1, minWidth: 0 }}>
                                    ...
                                    {log.details && (
                                        <div style={{ color: '#949ba4', fontSize: '12px', marginTop: '4px', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>

...
export default function useWatchParty(roomId, onClose) {
    ...
    const handleViewerJoined = useCallback(
        (data) => {
            toast.success(t('watchParty.userJoined', { username: data.username }));
            setViewers((prev) =>
                prev.find((v) => v.username === data.username)
                    ? prev
                    : [...prev, { username: data.username, id: data.user_id }]
            );
        },
        [t]
    );
    ...
    const connectWebSocket = useCallback(
        (partyId) => {
            ...
            const ws = new WebSocket(
                `${WS_PROTOCOL}://${API_HOST}/ws/watch-party/${partyId}/?token=${token}`
            );
            ...
            ws.onerror = (err) => logger.error('📺 Watch Party WebSocket error:', err);
    ...
    const createWatchParty = async () => {
        if (!videoUrl.trim()) {
            toast.error(t('watchParty.videoUrlRequired'));
            return;
        }
        setIsLoading(true);
        try {
            ...
            if (res.ok) {
                ...
                toast.success(t('watchParty.created'));
            } else {
                const err = await res.json();
                toast.error(err.error || t('watchParty.createFailed'));
            }
        } catch {
            toast.error(t('watchParty.connectionError'));
        } finally {
            setIsLoading(false);
        }
    };
    ...
    const joinWatchParty = async (partyId) => {
        ...
        try {
            ...
            if (res.ok) {
                const partyRes = await fetch(`${API_BASE_URL}/watch-party/${partyId}/`, {
                    headers: { Authorization: `Bearer ${token}` },
                });
                ...
                toast.success(t('watchParty.joined'));
            } else {
                const err = await res.json();
                toast.error(err.error || t('watchParty.createFailed'));
            }
    ...
    const sendSync = (action) => {
        ...
        wsRef.current.send(
            JSON.stringify({ type: 'sync', action, current_time: currentTime, playback_rate: 1.0 })
        );
    };
    ...
    const sendMessage = () => {
        ...
        wsRef.current.send(
            JSON.stringify({ type: 'message', content: newMessage, video_time: currentTime })
        );
        setNewMessage('');
    };
    ...
    const endParty = async () => {
        ...
        try {
            ...
            logger.error('End party error:', err);
        }
    };

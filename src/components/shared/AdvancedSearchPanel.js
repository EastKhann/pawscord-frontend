...
const AdvancedSearchPanel = ({ fetchWithAuth, apiBaseUrl, onClose, serverId, roomSlug }) => {
    ...
    const search = async () => {
        if (!query.trim()) {
            toast.error(t('search.enterQuery', 'Please enter a search query'));
        ...
        try {
            ...
            toast.success(t('search.foundResults', `Found ${data.results?.length || 0} results`));
        } catch (error) {
        ...
        <div style={styles.overlay}>
            <div style={styles.modal}>
                ...
                <div style={styles.searchBar}>
                    ...
                    >
                        <FaSearch />{' '}
                        {loading ? t('search.searching', 'Aranıyor...') : t('search.search', 'Ara')}
                    </button>
                </div>
                    ...
                    <div style={styles.checkboxGrid}>
                        ...
                        <label style={styles.checkbox}>
                            <input
                                ...
                                onChange={(e) =>
                                    setFilters({ ...filters, hasEmbed: e.target.checked })
                                }
                            />

                            {t('search.hasEmbed', 'Has Embed')}
                        </label>
                    </div>
                </div>
                ...
                <div style={styles.content}>
                    {loading ? (
                        <div style={styles.loading}>{t('search.searching', 'Aranıyor...')}</div>
                    ) : results.length === 0 ? (
                        <div style={styles.empty}>
                            {query
                                ? t('search.noResults', 'No results found')
                                : t('search.enterQueryPrompt', 'Enter a query and press search')}
                        </div>
                    ) : (
                        <div style={styles.resultsList}>
                            {results.map((result, idx) => (
                                <div key={`item-${idx}`} style={styles.resultCard}>
                                    <div style={styles.resultHeader}>
                                        <span style={styles.resultAuthor}>
                                    ...
                                    {result.room_name && (
                                        <div style={styles.resultMeta}>#{result.room_name}</div>
                                    )}
                                        ...
                                        <div style={styles.resultAttachments}>
                                            {result.has_image && (
                                                <span style={styles.badge}>
                                                    📷 {t('search.image', 'Image')}
                                                </span>
                                            )}

                                            {result.has_video && (
                                                <span style={styles.badge}>
                                                    🎥 {t('search.video', 'Video')}
                                                </span>
                                            )}

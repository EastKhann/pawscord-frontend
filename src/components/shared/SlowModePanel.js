...
/**
 * ⏱️ Slow Mode Panel
 * Channel message hızını sınırlama
 */
...
const SlowModePanel = ({ fetchWithAuth, apiBaseUrl, roomSlug, onClose }) => {
    ...
    const loadSlowMode = async () => {
        try {
            ...
            logger.error(t('ui.slow_mode_load_hatasi'), error);
        } finally {
            setLoading(false);
        }
    };
    ...
    const saveSlowMode = async () => {
        try {
            ...
            if (response.ok) {
                toast.success(t('ui.slow_mode_ayarları_kaydedildi'));
                onClose();
            } else {
                toast.error(t('common.saveFailed'));
            }
        } catch (error) {
            logger.error('Slow mode save error:', error);

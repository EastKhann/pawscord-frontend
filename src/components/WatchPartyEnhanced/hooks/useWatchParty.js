...
const useWatchParty = ({ roomId, currentUser, websocket, initialVideoUrl }) => {
    ...
    const sendWsControl = useCallback(
        (action, extra = {}) => {
            if (isHost && websocket) {
                websocket.send(
                    JSON.stringify({
    ...
    const requestSync = useCallback(() => {
        ...
        websocket.send(
            JSON.stringify({ type: 'watch_party_sync_request', roomId, sender: currentUser })
        );
    }, [websocket, roomId, currentUser]);
    ...
    useEffect(() => {
        ...
        const handleMessage = (event) => {
            try {
                ...
                logger.error('Watch Party WS error:', error);
            }
        };

...
const useSpamDetection = ({ serverId, fetchWithAuth, apiBaseUrl }) => {
    ...
    const fetchSettings = useCallback(async () => {
        ...
        try {
            const response = await fetchWithAuth(
                `${baseUrl}/api/servers/${serverId}/spam-settings/`
            );
            if (response.ok) {
            ...
            logger.error('Failed to fetch spam settings:', err);
        }

        try {
            const statsResponse = await fetchWithAuth(
                `${baseUrl}/api/servers/${serverId}/spam-stats/`
            );
            if (statsResponse.ok) {
            ...
            logger.error('Failed to fetch spam stats:', err);
            setStats(DEFAULT_STATS);
        }
    ...
    const handlePatternToggle = (pattern) => {
        setSettings((prev) => ({
            ...prev,
            patterns: { ...prev.patterns, [pattern]: !prev.patterns[pattern] },
        }));
    };
    ...
    const saveSettings = async () => {
        try {
            ...
            if (response.ok) toast.success(t('spam.saved'));
            else toast.error(t('spam.saveFailed'));
        } catch (err) {

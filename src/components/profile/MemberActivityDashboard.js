        ...
        try {
            ...
            if (fetchWithAuth && serverId) {
                const response = await fetchWithAuth(
                    `${baseUrl}/api/servers/${serverId}/member-activity/?range=${timeRange}`
                );
                if (response.ok) {
                ...
                {serverStats && (
                    <div className="stats-overview">
                        <div className="stat-card">
                            ...
                            >
                                {serverStats.message_trend >= 0 ? '+' : ''}
                                {serverStats.message_trend}%
                            </span>
                        </div>
                        <div className="stat-card">
                            ...
                            >
                                {serverStats.voice_trend >= 0 ? '+' : ''}
                                {serverStats.voice_trend}%
                            </span>
                        </div>
                        <div className="stat-card">
                            ...
                            >
                                {serverStats.reaction_trend >= 0 ? '+' : ''}
                                {serverStats.reaction_trend}%
                            </span>
                        </div>
                    </div>
                )}
                            ...
                            <div className="detail-header">
                                ...
                                <div className="member-info">
                                    ...
                                    <span className="last-active">
                                        Last active:{' '}
                                        {formatLastActive(selectedMember.stats.last_active)}
                                    </span>
                                </div>
                                    ...
                                    {selectedMember.activity_history.map((val, i) => (
                                        <div key={`item-${i}`} className="chart-column">
                                            <div
                                                ...
                                                style={_s({
                                                    height: `${(val / Math.max(...selectedMember.activity_history)) * 100}%`,
                                                })}
                                            >
...
MemberActivityDashboard.propTypes = {

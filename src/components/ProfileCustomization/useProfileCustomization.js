...
const useProfileCustomization = (userId, fetchWithAuth, apiBaseUrl) => {
    ...
    const loadCustomization = async () => {
        try {
            ...
            logger.error('Failed to load customization:', e);
        } finally {
            setLoading(false);
        }
    };
    ...
    const saveCustomization = async () => {
        try {
            ...
            if (r.ok) toast.success(t('profileCustomization.saved'));
        } catch (e) {
            logger.error('Failed to save customization:', e);
        }
    };
    ...
    const uploadBanner = async (file) => {
        ...
        try {
            const r = await fetchWithAuth(`${apiBaseUrl}/users/${userId}/upload-banner/`, {
                method: 'POST',
                body: formData,
            });
            const d = await r.json();
            ...
            logger.error('Failed to upload banner:', e);
        }
    };

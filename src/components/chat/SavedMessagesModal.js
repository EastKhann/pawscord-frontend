...
const SavedMessagesModal = ({
    ...
    const { overlayProps, dialogProps } = useModalA11y({ onClose, label: 'Saved Messages' });
    const [messages, setMessages] = useState([]);
    const [loading, setLoading] = useState(true);
    const removeActionButtonStyle = { ...styles.actionButton, color: '#f23f42' };
    ...
    const loadMessages = async () => {
        ...
        try {
            ...
            logger.error('Load saved messages error:', error);
        } finally {
            setLoading(false);
        }
    };
    ...
    const getTitle = () => {
        ...
        if (type === 'starred') return t('ui.starli_mesajlar');

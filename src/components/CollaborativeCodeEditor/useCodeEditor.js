...
const useCodeEditor = ({ roomId, userId, username, apiBaseUrl, fetchWithAuth, websocket }) => {
    ...
    const handleCollaboratorJoined = (data) =>
        setCollaborators((prev) => [
            ...prev,
            { id: data.user_id, username: data.username, color: data.color || '#5865f2' },
        ]);
    ...
    const loadSavedCode = async () => {
        try {
            ...
            logger.error('Failed to load code:', e);
        }
    };
    ...
    const broadcastCodeChange = (newCode, changes) => {
        ...
        websocket.send(
            JSON.stringify({
                ...
                username,
                code: newCode,
                changes: changes.map((c) => ({ range: c.range, text: c.text })),
            })
        );
    };
    ...
    const saveCode = async () => {
        setSaving(true);
        try {
            const res = await fetchWithAuth(`${apiBaseUrl}/code-editor/save/`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ room_id: roomId, code, language }),
            });
            if (res.ok) toast.success(t('codeEditor.saved'));
        } catch (e) {
            logger.error('Save error:', e);
            toast.error(t('codeEditor.saveFailed'));
        } finally {
            setSaving(false);
        }
    };
    ...
    const runCode = async () => {
        ...
        try {
            const res = await fetchWithAuth(`${apiBaseUrl}/code-editor/run/`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
    ...
    const copyCode = () => {
        ...
        toast.success(t('codeEditor.copied'));
    };
    ...
    const changeLanguage = (lang) => {
        ...
        if (window.monacoEditor && monacoRef.current)
            monacoRef.current.editor.setModelLanguage(window.monacoEditor.getModel(), lang);
    };

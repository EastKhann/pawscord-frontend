...
const ModeratorTools = ({ onClose, fetchWithAuth, apiBaseUrl, roomSlug, serverId }) => {
    ...
    const textAreaStyle = { ...styles.input, minHeight: '80px', resize: 'vertical' };

    const handleBanUser = async () => {
        if (!selectedUser) {
            toast.error(t('ui.please_select_a_user'));
            return;
        }
        ...
        try {
            ...
            if (res.ok) {
                toast.success(t('ui.user_yasaklandi'));
                setSelectedUser('');
                setBanReason('');
            } else {
                toast.error(t('ui.ban_failed'));
            }
        } catch (error) {
    ...
    const handleAddNote = async () => {
        if (!selectedUser || !moderatorNote) {
            toast.error(t('moderatorTools.userNoteRequired'));
            return;
        }
        ...
        try {
            ...
            if (res.ok) {
                toast.success(t('moderatorTools.noteAdded'));
                setModeratorNote('');
            }
        } catch (error) {

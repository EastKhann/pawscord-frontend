 ...
 */
const SafetyNumberModal = ({ username, targetUser, apiBaseUrl, fetchWithAuth, onClose }) => {
    ...
    const fetchSafetyNumber = async () => {
        try {
            const response = await fetchWithAuth(`${apiBaseUrl}/e2ee/safety-number/${targetUser}/`);
            ...
            logger.error('Failed to fetch safety number:', err);
            setLoading(false);
        }
    };
    ...
    const handleVerify = async () => {
        try {
            ...
            toast.error(t('ui.verification_failed'));
        }
    };
    ...
    return (
        <div style={styles.overlay} {...overlayProps}>
            <div style={styles.modal} {...dialogProps}>
                ...
                <div style={styles.user}>{targetUser}</div>

                {!showQR ? (
                    <>
                        <div style={styles.numberContainer}>
                            {safetyNumber?.split(' ').map((group, i) => (
                                <div key={`item-${i}`} style={styles.numberGroup}>
                                    {group}
                                </div>
                            ))}
                        </div>

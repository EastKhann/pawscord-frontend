...
export default function useMessageHandlers({
            ...
            const applyServerAck = (ackData) => {
                ...
                setMessages((prev) => {
                    const idx = prev.findIndex(
                        (m) => m.temp_id === payload.temp_id || m.id === payload.temp_id
                    );
                    ...
                    if (!httpResult?.ok) {
                        ...
                        if (!retryResult?.ok) {
                            ...
                            if (offlineQueue.enqueue(payload)) {
                                toast.info(t('chat.messageQueuedOffline'));
                            } else {
        ...
        async (data) => {
            ...
            if (!sent) {
                ...
                try {
                    ...
                    toast.error(t('chat.sendSnippetFailed'));
                }
            }
    ...
    const fetchMessageHistory = useCallback(
        async (isInitial = true, silent = false) => {
            ...
            try {
                ...
                logger.error('❌ [fetchMessageHistory] Error:', e);
            }
    ...
    const handleDeleteMessage = useCallback(
        async (messageId) => {
            ...
            try {
                const res = await fetchWithAuth(`${API_BASE_URL}/messages/${messageId}/delete/`, {
                    method: 'DELETE',
                });
    ...
    const handleTogglePin = useCallback(
        async (messageId) => {
            try {
                const res = await fetchWithAuth(`${API_BASE_URL}/messages/${messageId}/pin/`, {
                    method: 'POST',
                });
                if (res.ok) {
                    const data = await res.json();
                    ...
                    if (isPinned) {
                        ...
                        toast.success(t('chat.pinRemoved'));
                    }
                }
            } catch (e) {
                logger.error('Pin toggle error:', e);
    ...
    const handleSummarize = useCallback(async () => {
        ...
        setSummaryResult('');
        try {
            const res = await fetchWithAuth(`${API_BASE_URL}/rooms/${activeChat.id}/summarize/`, {
                method: 'POST',
            });
            const data = await res.json();
            setSummaryResult(res.ok ? data.summary : 'Hata: ' + data.error);
        } catch (e) {
            setSummaryResult('İstek sırasında bağlantı hatası oluştu.');
        }
    ...
    const handleClearChat = useCallback(async () => {
        if (
            ...
            return;
        try {
            const res = await fetchWithAuth(`${API_BASE_URL}/rooms/${activeChat.id}/clear/`, {
                method: 'POST',
            });
        ...
        async (conversationId) => {
            ...
            try {
                const res = await fetchWithAuth(
                    `${API_BASE_URL}/conversations/${conversationId}/hide/`,
                    { method: 'POST' }
                );
                if (res.ok) {
                    setConversations((prev) => prev.filter((c) => c.id !== conversationId));
                    if (activeChat.type === 'dm' && activeChat.id === conversationId)
                        window.dispatchEvent(new CustomEvent('setActiveChatWelcome'));
                }
            } catch (e) {
                logger.error(e);
            }
    ...
    const handleAdminDeleteConversation = useCallback(
        async (conversationId) => {
            if (
                !(await confirmDialog(
                    t(
            ...
            try {
                const res = await fetchWithAuth(
                    `${API_BASE_URL}/conversations/${conversationId}/admin-delete/`,
                    { method: 'DELETE' }
                );
                if (res.ok) {
                    const data = await res.json();
                    toast.success(
                        `${data.deleted_messages} ${t('chat.messagesDeleted', 'mesaj silindi')}. ${t('chat.participants', 'Katılımcılar')}: ${data.participants.join(', ')}`
                    );
                    setConversations((prev) => prev.filter((c) => c.id !== conversationId));
                    if (activeChat.type === 'dm' && activeChat.id === conversationId)
                        window.dispatchEvent(new CustomEvent('setActiveChatWelcome'));
                    ...
                    toast.error(errorData.error || t('chat.deleteFailed'));
                }
            } catch (e) {
                logger.error(e);
    ...
    const prefetchMessages = useCallback(
        async (type, id) => {
            ...
            // Zaten cache'de varsa or uçuşta ise atla

...
export default function useFileUpload({
    ...
    const uploadFile = useCallback(
        async (file, isVoice = false, duration = 0, targetOverride = null) => {
            ...
            try {
                        ...
                        setMessages((prev) => {
                            if (initData.temp_id) {
                                const idx = prev.findIndex(
                                    (msg) => msg.temp_id === initData.temp_id
                                );
                                if (idx !== -1) {
                                    const n = [...prev];
                                    n[idx] = initData;
                ...
                const uploadPart = async (partNumber) => {
                    ...
                    const uploadRes = await fetchWithAuth(
                        `${API_BASE_URL}/upload/multipart/upload-part/`,
                        {
                            method: 'POST',
                            body: formData,
                            headers: {},
                    ...
                    completedParts++;
                    if (showProgress)
                        setUploadProgress(Math.round((completedParts / totalParts) * 95));
                ...
                logger.error('❌ [R2 Multipart] Error:', e);
                toast.error(t('chat.uploadError', { message: e.message }));
            }
    ...
    const startVoiceRecording = useCallback(async () => {
        try {
            ...
            if (error.name === 'NotAllowedError') toast.warning(t('chat.micDenied'));
            else if (error.name === 'NotFoundError') toast.warning(t('chat.micNotFound'));
            else toast.error(t('chat.micError', { message: error.message }));
        }
    }, [uploadFile]);

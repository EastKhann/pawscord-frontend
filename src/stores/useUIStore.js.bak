...
export const useUIStore = create(
    devtools(
        ...
        { name: 'UIStore', store: 'useUIStore' }
    )
);

// --- SELECTORS (prevent unnecessary re-renders) ---
export const useIsModalOpen = (name) => useUIStore((s) => s.modals[name] ?? false);
export const useModalData = (name) => useUIStore((s) => s.modalData[name]);
export const useTheme = () => useUIStore((s) => s.theme);
export const useAccentColor = () => useUIStore((s) => s.accentColor);
export const useConnectionStatus = () => useUIStore((s) => s.connectionStatus);
export const usePanelVisible = (name) => useUIStore((s) => s.panels[name] ?? true);
export const useToastNotifications = () => useUIStore((s) => s.toastNotifications);

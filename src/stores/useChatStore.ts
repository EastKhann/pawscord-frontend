...
export const useChatStore = create<ChatStore>()(
    devtools(
        (set, get) => ({
            ...
            addMessage: (message: any) =>
                set((state: any) => {
                    if (!message || typeof message !== 'object' || !message.id) {
                        logger.warn('[Store] Invalid message rejected:', message);
                        return state;
                    }
                    // Replace optimistic message by temp_id
                    if (message.temp_id) {
                        const exists = state.messages.some(
                            (m: any) => m.temp_id === message.temp_id
                        );
                        if (exists) {
                            return {
                                messages: state.messages.map((m: any) =>
                                    m.temp_id === message.temp_id ? message : m
                                ),
                            };
                        }
                    }
            ...
            updateMessage: (id: any, updates: any) =>
                set((state: any) => ({
                    messages: state.messages.map((m: any) =>
                        m.id === id ? { ...m, ...updates } : m
                    ),
            ...
            prependMessages: (oldMessages: any[]) =>
                set((state: any) => {
                    // Dedup against existing
                    const existingIds = new Set(state.messages.map((m: any) => m.id));
                    const toAdd = oldMessages.filter(
                        (m: any) => m && m.id && !existingIds.has(m.id)
                    );
            ...
            setTypingUser: (username: string, isTyping: boolean) =>
                set((state: any) => {
                    ...
                    entries = entries.filter(
                        (e: TypingEntry) => now - e.timestamp < TYPING_TIMEOUT_MS
                    );

                    if (isTyping) {
                        const existing = entries.findIndex(
                            (e: TypingEntry) => e.username === username
                        );
                        if (existing >= 0) {
            ...
            // --- SELECTION (array for JSON serializability) ---
            selectedMessages: [] as string[],
            setSelectedMessages: (val: any) =>
                set({
                    selectedMessages: typeof val === 'function' ? val(get().selectedMessages) : val,
                }),

            // --- ENCRYPTION ---
            setEncryptionKey: (chatId: string, key: any) =>
                set((state: any) => ({
                    encryptionKeys: {
...
export const useLastMessage = () => useChatStore((s) => s.messages[s.messages.length - 1]);
export const useHasUnread = () =>
    useChatStore((s) => Object.values(s.unreadCounts || {}).some((c) => c > 0));
export const useOnlineUserCount = () => useChatStore((s) => s.onlineUsers.length);

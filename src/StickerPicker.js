...
const StickerPicker = ({ categoryId, onSelect, onClose, fetchWithAuth, apiBaseUrl }) => {
    ...
    const categories = useMemo(() => {
        const cats = new Set();
        stickers.forEach((s) => { if (s.category) cats.add(s.category); });
    ...
    const visible = useMemo(() => {
        ...
        if (search.trim()) {
            ...
            list = list.filter((s) => (s.name || '').toLowerCase().includes(q) || (s.tags || '').toLowerCase().includes(q));
        }
        return list;
    }, [stickers, tab, search]);
    ...
    return (
        <div ref={containerRef} style={styles.container}>
            ...
            <div style={styles.header}>
                <span style={styles.headerTitle}>Stickers</span>
            ...
            <div style={styles.tabs}>
                {categories.map((cat) => (
                    <button
                        ...
                        onClick={() => setTab(cat)}
                    >
                        {cat === 'all' ? '✦ All' : cat === 'recent' ? '🕐 Recent' : cat}
                    </button>
                ))}
            </div>
            ...
            <div style={styles.grid}>
                {loading ? (
                    <div style={styles.loadingWrap}>
                        ...
                        <span style={styles.loadingText}>Loading stickers...</span>
                    </div>
                ) : visible.length > 0 ? (
                    visible.map((sticker) => (
                        <div
                    ...
                    <div style={styles.emptyWrap}>
                        ...
                        <p style={styles.emptyText}>
                            {search ? 'No stickers match your search' : tab === 'recent' ? 'No recent stickers yet' : 'No stickers available'}
                        </p>
                    </div>
                )}
            </div>
...
const styles = {
    container: {
        position: 'absolute', bottom: 60, right: 10, width: 340, height: 380,
    ...
    header: {
        ...
        padding: '10px 14px', borderBottom: '1px solid #1e2024',
    },
    ...
    searchInput: {
        ...
        color: '#ddd', fontSize: '0.85em', outline: 'none', boxSizing: 'border-box',
    },
    tabs: {
        display: 'flex', gap: 4, padding: '6px 14px', overflowX: 'auto',
        borderBottom: '1px solid #1e2024', flexShrink: 0,
    },
    tabBtn: {
        flexShrink: 0, padding: '4px 10px', borderRadius: 4, border: 'none',
        backgroundColor: 'transparent', color: '#949ba4', cursor: 'pointer',
        fontSize: '0.78em', whiteSpace: 'nowrap', transition: 'all 0.15s',
    },
    tabBtnActive: { backgroundColor: '#5865f2', color: '#fff' },
    grid: {
        flex: 1, padding: 10, display: 'grid',
        gridTemplateColumns: 'repeat(4, 1fr)', gap: 8, overflowY: 'auto',
        alignContent: 'start',
    },
    stickerWrap: {
        aspectRatio: '1', display: 'flex', alignItems: 'center',
        justifyContent: 'center', borderRadius: 6, cursor: 'pointer',
        backgroundColor: '#1e2024', transition: 'transform 0.12s, background 0.12s',
    },
    ...
    preview: {
        ...
        backgroundColor: '#1e2024', borderRadius: 8, padding: '10px 14px',

...
const EnglishVoicePractice = ({ apiBaseUrl }) => {
    ...
    const drawWave = useCallback(() => {
        ...
        const samples = Array.from({ length: bars }, (_, i) => Math.max(3, Math.round((data[i * step] / 255) * 48)));
        setWaveData(samples);
    ...
    return (
        <div style={S.container}>
            ...
            <div style={S.header}>
                <Link to="/eng-learn" style={S.backBtn}><FaArrowLeft /> Geri</Link>
                <h2 style={{ margin: 0, fontSize: '1.1em', color: '#dbdee1' }}>🎙️ AI ile Konuş</h2>
                <div style={{ width: 60 }} />
            </div>
            ...
            <div style={S.chatArea}>
                {messages.length === 0 && !processing && (
                    <div style={S.emptyState}>
                        <div style={{ fontSize: '4.5em', marginBottom: 14, filter: 'drop-shadow(0 6px 20px rgba(88,101,242,0.4))' }}>🤖</div>
                        <h3 style={{ color: '#f2f3f5', margin: '0 0 8px', fontWeight: 800, fontSize: '1.15em' }}>İngilizce Pratik Yap</h3>
                        <p style={{ color: '#6b7280', margin: '0 0 22px', fontSize: '0.9em', lineHeight: 1.6 }}>Mikrofon butonuna basılı tut ve konuşmaya başla.<br />Ya da aşağıdaki konulardan birini seç:</p>
                        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8, justifyContent: 'center', maxWidth: 440 }}>
                            {STARTER_PROMPTS.map((p, i) => (
                                <div key={i} style={S.promptChip}>
                                    <FaLightbulb size={10} style={{ marginRight: 4 }} />{p}
                                </div>
                            ))}
                        </div>
                    </div>
                ...
                {messages.map((msg, i) => (
                    <div key={i} style={{ display: 'flex', flexDirection: 'column', alignItems: msg.sender === 'user' ? 'flex-end' : 'flex-start', gap: 3 }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 5, opacity: 0.7, fontSize: '0.75em', color: '#949ba4', padding: '0 4px' }}>
                            {msg.sender === 'user' ? <FaUser size={10} /> : <FaRobot size={10} />}
                            {msg.sender === 'user' ? 'Sen' : 'AI Öğretmen'}
                        </div>
                        <div style={{
                            ...S.bubble,
                            ...
                            border: msg.sender === 'ai' ? '1px solid rgba(88,101,242,0.18)' : 'none',
                ...
                {processing && (
                    <div style={{ display: 'flex', alignItems: 'center', gap: 6, color: '#949ba4', fontSize: '0.85em', padding: '4px 8px' }}>
                        <FaRobot size={12} />
                        {[0, 1, 2].map(i => (
                            <span key={i} style={{ width: 6, height: 6, borderRadius: '50%', background: '#5865f2', display: 'inline-block', animation: `typingDot 1.2s ${i * 0.2}s ease infinite` }} />
                        ))}
                    </div>
                )}
            ...
            <div style={S.controls}>
                {isRecording && (
                    <div style={S.waveWrapper}>
                        {waveData.map((h, i) => (
                            <div key={i} style={{ ...S.waveBar, height: h, background: `hsl(${230 + i * 2}, 80%, 60%)` }} />
                        ))}
                    </div>
                )}
                ...
                <div style={{ position: 'relative', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                    {isRecording && [1, 2].map(r => (
                        <span key={r} style={{
                            position: 'absolute', borderRadius: '50%', border: '2px solid rgba(242,63,66,0.4)',
                            width: 80 + r * 30, height: 80 + r * 30,
                ...
                <p style={{ marginTop: 14, color: '#949ba4', fontSize: '0.85em', textAlign: 'center' }}>
...
const S = {
    container: {
        ...
        color: 'white', boxSizing: 'border-box', fontFamily: 'Poppins, sans-serif'
    },
    header: {
    ...
    backBtn: {
        color: '#949ba4', fontSize: '0.9em', textDecoration: 'none',
    ...
    chatArea: {
        ...
        gap: 14, padding: '18px 20px 22px'
    },
    emptyState: {
        display: 'flex', flexDirection: 'column', alignItems: 'center',
        justifyContent: 'center', flex: 1, textAlign: 'center', paddingTop: 32
    },
    promptChip: {
        background: 'rgba(88,101,242,0.08)',
        border: '1px solid rgba(88,101,242,0.25)',
        borderRadius: 20, padding: '7px 14px', fontSize: '0.8em',
        color: '#9ba5ff', display: 'flex', alignItems: 'center', cursor: 'default',
        transition: 'background 0.15s'
    },
    bubble: {
        maxWidth: '84%', padding: '13px 17px',
        borderRadius: 18, fontSize: '0.95em', lineHeight: 1.65,

...
const FloatingVoiceIsland = ({
    ...
    const handleResizeStop = useCallback(
        (e, data) => {
            e.stopPropagation();
            setIsInteracting(false);
            if (onResize) onResize(data.size);
        },
        [onResize]
    );
                ...
                <div aria-label={t('aria.floatingVoiceIsland', 'Voice Island')} ref={nodeRef} className={animationClass}>
                    {isMinimized ? (
                        ...
                        >
                            ...
                            {isInteracting && <div style={styles.interactionShield} />}
                        </ResizableBox>
                    )}
                </div>
            </Draggable>
...
const styles = {
    ...
    islandContainer: {
        ...
        borderRadius: '16px',
        boxShadow:
            '0 8px 40px rgba(0, 0, 0, 0.7), 0 0 20px rgba(88, 101, 242, 0.2), inset 0 1px 0 rgba(255, 255, 255, 0.05)',
        display: 'flex',
...
FloatingVoiceIsland.propTypes = {
    islandState: PropTypes.bool,

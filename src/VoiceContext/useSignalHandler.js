    ...
    const handleSignalMessage = useCallback(
        async (data) => {
            ...
            if (data.type === 'server_muted') {
                toast.warning(data.message || t('signal.micMuted'), 5000);
            ...
            if (data.type === 'kicked') {
                logger.warn('🔴 [Voice] Kicked from channel:', data.reason, data.message);

                if (data.reason === 'moved' && data.target_channel) {
                    toast.info(data.message || t('signal.moved'), 3000);
            ...
            if (data.type === 'track_metadata') {
                if (trackMetadataRef && data.trackId && data.streamType) {
                    ...
                    logger.webrtc(
                        `[TrackMetadata] ${data.from || 'peer'} trackId=${data.trackId} → ${data.streamType}`
                    );
                }
                return;
            }
            ...
            if (data.type === 'play_sound') {
                ...
                if (soundUrl) {
                    try {
                        ...
                        audio.volume = 0.5;
                        audio.play().catch((e) => logger.warn('[Soundboard] Autoplay blocked:', e));
                        logger.webrtc(`[Soundboard] Playing "${soundUrl}" from ${senderUsername}`);
                    } catch (e) {
                        logger.warn('[Soundboard] Failed to play remote sound:', e);
                    }
                }
                return;
            }
            ...
            if (data.type === 'recording_state') {
                ...
                if (data.is_recording) {
                    toast.warning(t('signal.recording', { user: senderUsername }), 5000);
                } else {
                    toast.info(t('signal.recordingStopped', { user: senderUsername }), 3000);
                }
            ...
            if (data.type === 'stage_signal') {
                ...
                if (action === 'request_speak') {
                    toast.info(t('signal.speakRequest', { user: senderUsername }), 3000);
                } else if (action === 'grant_speak' && targetUsername === username) {
                    toast.success(t('signal.speakGranted'), 3000);
                    ...
                    toast.warning(t('signal.speakRevoked'), 3000);
                    if (localStreamRef.current) {
                        localStreamRef.current.getAudioTracks().forEach((track) => {
                            track.enabled = false;
                        });
                ...
                setConnectedUsers((prev) => {
                    if (action === 'grant_speak') {
                        return prev.map((u) =>
                            u.username === targetUsername ? { ...u, isSpeaker: true } : u
                        );
                    } else if (action === 'revoke_speak') {
                        return prev.map((u) =>
                            u.username === targetUsername ? { ...u, isSpeaker: false } : u
                        );
                    }
                    return prev;
                });
            ...
            if (!senderUsername) {
                logger.error('[Signal] No sender username found in message:', data);
                return;
            }
            ...
            if (!pc) {
                if (type === 'offer') {
                    ...
                    try {
                        ...
                        logger.error(
                            `❌ [user_joined] Failed to create/send offer to ${senderUsername}:`,
                            e
                        );
                    }
                    return;
                        ...
                        prev.map((u) => {
                            if (u.username === senderUsername) {
                                return {
                                    ...u,
                                    isCameraOn:
                                        data.streamType === 'camera' ? data.enabled : u.isCameraOn,
                                    isScreenSharing:
                                        data.streamType === 'screen'
                                            ? data.enabled
                                            : u.isScreenSharing,
                                };
                            }
                    ...
                    if (data.enabled && !peerConnectionsRef.current[senderUsername]) {
                        ...
                        try {
                            ...
                            logger.signal(
                                `Sent optimized offer to ${senderUsername} after stream_update`
                            );
                        } catch (e) {
                    ...
                    if (type === 'candidate') {
                        ...
                        iceCandidateBufferRef.current[senderUsername].push(
                            new RTCIceCandidate(candidate)
                        );
                        logger.signal(`Buffered candidate for ${senderUsername} (no PC yet)`);
                        return;
                    }
                    logger.warn(`[Signal] Ignored ${type} from ${senderUsername} (No PC)`);
                    return;
                }
            }
            ...
            try {
                if (type === 'offer') {
                    if (pc.signalingState !== 'stable') {
                        ...
                        if (isPolite) {
                            logger.signal(
                                `[Signal] Offer collision with ${senderUsername} — rolling back (polite peer)`
                            );
                            await pc.setLocalDescription({ type: 'rollback' });
                        } else {
                            logger.signal(
                                `[Signal] Offer collision with ${senderUsername} — ignoring (impolite peer)`
                            );
                            return;
                        }
                    }

                    // 🔥 Gelen offer'daki Opus settingsını da optimize et
                    const optimizedRemoteSdp = { ...sdp, sdp: optimizeOpusSdp(sdp.sdp || sdp) };
                    await pc.setRemoteDescription(
                        new RTCSessionDescription(
                            optimizedRemoteSdp.type ? optimizedRemoteSdp : sdp
                        )
                    );
                    ...
                    if (pc.signalingState !== 'have-local-offer') {
                        logger.warn(
                            `[WebRTC] Error handling answer from ${senderUsername}: InvalidStateError: Failed to execute 'setRemoteDescription' on 'RTCPeerConnection': Failed to set remote answer sdp: Called in wrong state: ${pc.signalingState}`
                        );
                        return;
                    }
                ...
                logger.error(`[WebRTC] Error handling ${type} from ${senderUsername}:`, e);
            }
        },
        [
            username,

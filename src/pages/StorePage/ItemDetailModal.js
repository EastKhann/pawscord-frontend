...
const ItemDetailModal = ({ item, isOwned, onPurchase, onClose }) => {
  ...
  return (
    <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,0.85)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 10000 }} {...overlayProps}>
      <div style={{ background: 'linear-gradient(135deg, #111214 0%, #0d0e10 100%)', borderRadius: '20px', padding: '40px', maxWidth: '500px', width: '90%', border: `3px solid ${getRarityColor(item.rarity)}`, boxShadow: `0 0 40px ${getRarityColor(item.rarity)}60` }} onClick={(e) => e.stopPropagation()} {...dialogProps}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: '30px' }}>
          ...
          {!isOwned && (
            <button onClick={() => onPurchase(item)} style={{ background: 'linear-gradient(135deg, #23a559 0%, #3ca374 100%)', border: 'none', color: '#fff', padding: '12px 32px', borderRadius: '10px', cursor: 'pointer', fontWeight: 700, fontSize: '16px' }}>

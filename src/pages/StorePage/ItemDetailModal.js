...
const ItemDetailModal = ({ item, isOwned, onPurchase, onClose }) => (
  <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,0.85)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 10000 }} onClick={onClose}>
    <div style={{ background: 'linear-gradient(135deg, #2c2f33 0%, #23272a 100%)', borderRadius: '20px', padding: '40px', maxWidth: '500px', width: '90%', border: `3px solid ${getRarityColor(item.rarity)}`, boxShadow: `0 0 40px ${getRarityColor(item.rarity)}60` }} onClick={(e) => e.stopPropagation()}>

...
const _st1 = { marginLeft: '2px' };
const _st2 = { display: 'flex', alignItems: 'center', gap: '6px' };
const _st3 = { marginRight: '4px' };
const _st1049 = {
    background: 'none',
    border: 'none',
    color: '#b5bac1',
...
const WELCOME_SCREEN_STYLE_MARKER = 'data-welcome-screen-animations';
const WELCOME_SCREEN_ANIMATIONS = `
...
const injectWelcomeScreenAnimations = () => {
    if (typeof document === 'undefined' || !document.head) return;
    if (document.head.querySelector(`style[${WELCOME_SCREEN_STYLE_MARKER}]`)) return;

    const styleSheet = document.createElement('style');
    styleSheet.setAttribute(WELCOME_SCREEN_STYLE_MARKER, 'true');
    styleSheet.textContent = WELCOME_SCREEN_ANIMATIONS;
...
const WelcomeScreen = ({
    ...
    const getGreeting = () => {
        ...
        if (hour >= 18 && hour < 22) return t('home.greeting_evening');
    ...
    const UpdateButtonComponent = () => {
        ...
        if (isDownloading) {
            return (
                <div style={styles.headerProgressContainer} title={updateStatusText}>
                    <div
                        style={{ ...styles.headerProgressBarFill, width: `${downloadProgress}%` }}
                    ></div>
        ...
        return (
            ...
            >
                <FaDownload size={14} />
                <span style={_st1}>{t('home.update_btn')}</span>
            </button>
        );
    };
    ...
    return (
        <div style={styles.container}>
            {showDownload && (
                <DownloadModal onClose={() => setShowDownload(false)} apiBaseUrl={API_BASE_URL} />
            )}
            ...
            {/* --- DESKTOP DOWNLOAD BUTTONS (TOP RIGHT) --- */}
            {!isMobile && (
                <div style={styles.desktopTopRight}>
                    ...
                    {shouldShowDownloadBtn && (
                        ...
                        >
                            <FaDownload style={_st3} /> {t('common.download', 'İndir')}
                        </button>
                    )}
                </div>
            )}
            ...
            >
                ...
                {recentItems.length > 0 && (
                    <div style={styles.recentSection}>
                        <p style={styles.recentTitle}>{t('home.recentlyOpened')}</p>
                ...
                <div style={styles.cardsGrid}>
                    ...
                    >
                        ...
                        <h3 style={styles.cardTitle}>{t('home.friends_title')}</h3>
                        <p style={styles.cardDesc}>{t('home.friends_desc')}</p>
                    </div>
                    ...
                    >
                        ...
                        <h3 style={styles.cardTitle}>{t('home.servers_title')}</h3>
                        <p style={styles.cardDesc}>{t('home.servers_desc')}</p>
                    </div>
                    ...
                    >
                        ...
                        <h3 style={styles.cardTitle}>{t('home.activities_title')}</h3>
                        <p style={styles.cardDesc}>{t('home.activities_desc')}</p>
                    </div>
                    ...
                    >
                        ...
                        <h3 style={styles.cardTitle}>{t('home.ai_title')}</h3>
                        <p style={styles.cardDesc}>{t('home.ai_desc')}</p>
                    </div>
                </div>

                <div style={styles.footer}>
                    <p>&copy; {new Date().getFullYear()} Pawscord</p>
                </div>
            </div>
        </div>
    );
...
const styles = {
    container: {
        ...
        backgroundImage:
            'radial-gradient(ellipse at 20% 0%, rgba(88,101,242,0.14) 0%, transparent 50%), radial-gradient(ellipse at 80% 100%, rgba(59,91,219,0.08) 0%, transparent 45%)',
        color: '#dbdee1',
        display: 'flex',
    ...
    logo: {
        ...
        height: '96px',
        marginBottom: '20px',
        filter: 'drop-shadow(0 8px 28px rgba(88,101,242,0.42)) drop-shadow(0 2px 8px rgba(0,0,0,0.7))',
    },
    logoMobile: {
        width: '80px',
        height: '80px',
        marginBottom: '16px',
        filter: 'drop-shadow(0 6px 20px rgba(88,101,242,0.36)) drop-shadow(0 2px 6px rgba(0,0,0,0.6))',
    },
    title: {
        fontSize: '2.25em',
        fontWeight: '800',
        background: 'linear-gradient(135deg, #ffffff 20%, #b8b9c7 100%)',
...
WelcomeScreen.propTypes = {
    ...
    onOpenDiscovery: PropTypes.func,
    onOpenRightMenu: PropTypes.func,
    isDownloading: PropTypes.bool,
    downloadProgress: PropTypes.array,
    updateStatusText: PropTypes.func,
    onStartUpdate: PropTypes.func,
    onSwitchToAI: PropTypes.func,
    onSwitchToCinema: PropTypes.func,

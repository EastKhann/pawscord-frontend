...
(async () => {
  ...
  await page.evaluateOnNewDocument(() => {
    ...
    window.fetch = async (input, init) => {
      try {
        ...
        if (url.includes('/avatars') || url.includes('/default')) return new Response(JSON.stringify(['/avatars/1.png','/avatars/2.png']), {status:200, headers:{'Content-Type':'application/json'}});
  ...
  // try to click splash logo to advance
  const splash = await page.$('.splash-logo');
  if (splash) {
    try { await splash.click({ delay: 50 }); await sleep(800); } catch(e){}
  }

  await sleep(1200);

  const items = await page.evaluate(() => {
    const sel = Array.from(document.querySelectorAll('button, a[href], [role=button], [title], img, div'));
    const res = [];
    for (const el of sel) {
      try {
        ...
        res.push({ tag: el.tagName, text, title: el.getAttribute('title') || null, outerHTML: el.outerHTML.substring(0,500), x: Math.round(rect.x), y: Math.round(rect.y), w: Math.round(rect.width), h: Math.round(rect.height) });
      } catch(e){}
    }
    return res;
  });
  ...
  out.items = items;
  fs.writeFileSync(path.join(__dirname,'ui_clickables_after_dump.json'), JSON.stringify(out, null, 2));
  console.log('done');
  await browser.close();
})();
